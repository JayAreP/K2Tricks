...
function createGcpInstance {
    ...
    $o | Add-Member -MemberType NoteProperty -Name "machineType" -Value "projects/k2c-app-demo/zones/us-central1-f/machineTypes/n1-standard-2"
...
$number = 1
while ($number -le $numberOfInstances) {
    $instanceName = $namePrefix + '-' + $number.ToString()
    createGcpInstance -name $instanceName
    $number++
}